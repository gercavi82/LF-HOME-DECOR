...
export function InventoryExportMenu({ currentParams }: InventoryExportMenuProps) {
  ...
  const buildExportUrl = (formato: "xlsx" | "pdf" | "csv") => {
    ...
    return `/api/exportar/inventario?${params.toString()}`;
  };
        ...
        <div className="absolute right-0 z-50 mt-2 w-56 origin-top-right rounded-2xl border border-slate-100 bg-white p-2 shadow-xl ring-1 ring-black/5 animate-in fade-in zoom-in-95 duration-100">
          ...
          >
            ...
            <div>
              ...
              <p className="text-xs text-slate-400">Detalle y resumen de stock</p>
            </div>
          </a>
          ...
          >
            ...
            <div>
              ...
              <p className="text-xs text-slate-400">Exportación de datos plana</p>
            </div>
          </a>
        </div>
      )}
