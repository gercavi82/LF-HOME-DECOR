...
interface PurchasesExportMenuProps {
  currentParams: {
    q?: string;
    desde?: string;
    hasta?: string;
...
export function PurchasesExportMenu({ currentParams }: PurchasesExportMenuProps) {
  ...
  const buildExportUrl = (formato: "xlsx" | "pdf" | "csv") => {
    ...
    if (currentParams.proveedor) params.set("proveedor", currentParams.proveedor);
    if (currentParams.estadoPago) params.set("estadoPago", currentParams.estadoPago);
    if (currentParams.anio) params.set("anio", currentParams.anio);
    if (currentParams.mes) params.set("mes", currentParams.mes);
    if (currentParams.tipo) params.set("tipo", currentParams.tipo);

    return `/api/exportar/compras?${params.toString()}`;
  };
  ...
  return (
    <div className="relative inline-block text-left" ref={menuRef}>
      <button
        ...
        className="inline-flex h-9 items-center gap-1.5 rounded-xl border border-slate-200 bg-white px-3 text-xs font-semibold text-slate-700 shadow-sm transition hover:bg-slate-50 focus:outline-none focus:ring-2 focus:ring-slate-400/20"
        ...
        <div className="absolute right-0 z-50 mt-2 w-56 origin-top-right rounded-2xl border border-slate-100 bg-white p-2 shadow-xl ring-1 ring-black/5 animate-in fade-in zoom-in-95 duration-100">
          ...
          >
            ...
            <div>
              ...
              <p className="text-xs text-slate-400">Facturas y resumen FIFO</p>
            </div>
          </a>
          ...
          >
            ...
            <div>
              <p className="font-semibold leading-tight">CSV (Texto plano)</p>
